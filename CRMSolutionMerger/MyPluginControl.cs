...
namespace CRMSolutionMerger
{
    public partial class MyPluginControl : MultipleConnectionsPluginControlBase
    {
        ...
        private void CloneMergePatch(IOrganizationService service, DataGridViewRow row, string msg)
        {
            if(row == null)
            {
                return;
        ...
        private void tsbSClonePath_Click(object sender, EventArgs e)
        {
            if (selectedSRow != null)
            {
                ...
                CloneMergePatch(Service, selectedSRow, "Clone Patch - Source");
            }
            else if (selectedTRow != null)
            {
                if (targetService == null) AddAdditionalOrganization();

                CloneMergePatch(targetService, selectedTRow, "Clone Patch - Target");
            }
            else
            {
        ...
        private void tsbSMergePatch_Click(object sender, EventArgs e)
        {
            if (selectedSRow != null)
            {
                ...
                CloneMergePatch(Service, selectedSRow, "Merge Patch - Source");
            }
            else if (selectedTRow != null)
            {
                if (targetService == null) AddAdditionalOrganization();

                CloneMergePatch(targetService, selectedTRow, "Merge Patch - Target");
            }
            else
            {
                MessageBox.Show($"Select Source/Target row's first!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void tsbCopySource2Target_Click(object sender, EventArgs e)
        {
            ...
            if (selectedSRow != null)
                MoveSolution(Service, targetService, selectedSRow, "Source", "Target");
            else if (selectedTRow != null)
                MoveSolution(targetService, Service, selectedTRow, "Target", "Source");
        }

        private void MoveSolution(IOrganizationService service, IOrganizationService tService, DataGridViewRow row, string msg, string dMsg)
        {
            WorkAsync(new WorkAsyncInfo
            {
